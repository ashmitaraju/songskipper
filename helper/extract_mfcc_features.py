...
for filename in os.listdir("test_videos"):
    ...
    start_time = 0
    print(filename)
    duration = librosa.core.get_duration(filename = "test_videos\\" + filename)
    #while (start_time + interval <= duration):
    while (start_time <= duration):

        data, sample_rate = librosa.load( "test_videos\\" + filename, sr = None, mono = True, offset = start_time, duration = interval_length)
        mfcc = np.mean(librosa.feature.mfcc( y = data, sr = sample_rate, n_mfcc = 40).T,axis=0)
        
        # Append the start time of the frame to mfcc
        mfcc = np.append(mfcc, start_time)
        mfccs.append(mfcc)
        start_time += interval_length
       
pd.DataFrame(mfccs).to_csv("mfccs_test_features.csv")
